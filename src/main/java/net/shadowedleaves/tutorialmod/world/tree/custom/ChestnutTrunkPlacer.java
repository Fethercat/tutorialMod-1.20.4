package net.shadowedleaves.tutorialmod.world.tree.custom;
        ...
        for(int i = 0; i < height_; i++) {
            ...
            if(i % 2 == 0 && random.nextBoolean()) {
                if(random.nextFloat() > 0.25f) {
                    for(int x = 1; x <= 4; x++) {
                        replacer.accept(startPos.up(i).offset(Direction.NORTH, x), (BlockState) Function.identity()
                ...
                if(random.nextFloat() > 0.25f) {
                    for(int x = 1; x <= 4; x++) {
                        replacer.accept(startPos.up(i).offset(Direction.SOUTH, x), (BlockState) Function.identity()
                                .apply(config.trunkProvider
                ...
                if(random.nextFloat() > 0.25f) {
                    for(int x = 1; x <= 4; x++) {
                        replacer.accept(startPos.up(i).offset(Direction.EAST, x), (BlockState) Function.identity()
                                .apply(config.trunkProvider
                ...
                if(random.nextFloat() > 0.25f) {
                    for(int x = 1; x <= 4; x++) {
                        replacer.accept(startPos.up(i).offset(Direction.WEST, x), (BlockState) Function.identity()
                                .apply(config.trunkProvider
